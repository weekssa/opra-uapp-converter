...
ADDING_HEADPHONES = ROOT / "docs" / "ADDING_HEADPHONES.md"
...
Each version is published as a matching Git tag such as `v{version}` and a GitHub Release. The release includes generated release notes, a versioned ZIP of the complete UAPP preset library, a SHA-256 checksum for that ZIP, and the matching `output/manifest.json`.
...
def automation_versioning(version: str) -> str:
    return f"""## Automatic project versioning and releases
...
def update_adding_headphones_release_text(text: str) -> str:
    old = "6. update `output/manifest.json` and XML files only when actual output changed."
    if old not in text:
        if "publish a matching GitHub Release" in text:
            return text
        raise RuntimeError("ADDING_HEADPHONES.md is missing the expected build-step anchor")
    new = (
        old
...
def main() -> None:
    ...
    adding = ADDING_HEADPHONES.read_text(encoding="utf-8")
    adding = update_adding_headphones_release_text(adding)
    ADDING_HEADPHONES.write_text(adding, encoding="utf-8")
